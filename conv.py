...
class Conv3x3:
    ...
    def __init__(self, num_filters: int):
        ...
        #
        # np.random.rand(num_filters, 3, 3) 从标准分布中初始化出"num_filters"个3x3方阵作为过滤器
        #
    ...
    def forward(self, padded_image: NDArray) -> NDArray:
        ...
        for region, i, j in self.iterate_regions(padded_image):
            ...
            #
            # np.sum() calculates the result of the previous step using axis=(1, 2), which produces a 1d array
            # of length "num_filters" where each element contains the convolution result for the corresponding filter
